...
class Scoreboard {
	...
	const char* texture_path="../bin/images/scoreboard.png";
public:
	...
	bool render(GLuint program) {
		...
		glUniform2fv(glGetUniformLocation(program, "guiPosition"), 1, vec2(0,1.65f));
	...
	Scoreboard() {
		...
		vertex_list.clear();
		vertex_list.push_back({ vec3(-0.21f, -0.1f, 0.0f), vec3(0, 0, 1), vec2(0.0f, 0.0f) });
		vertex_list.push_back({ vec3(0.21f, -0.1f, 0.0f), vec3(0, 0, 1), vec2(1.0f, 0.0f) });
		vertex_list.push_back({ vec3(0.21f, 0.1f, 0.0f), vec3(0, 0, 1), vec2(1.0f, 1.0f) });
		vertex_list.push_back({ vec3(-0.21f, 0.1f, 0.0f), vec3(0, 0, 1), vec2(0.0f, 1.0f) });
